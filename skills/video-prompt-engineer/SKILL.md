...
3b. **From the style guide**, when one was used: which slug, and which clauses came from it
   verbatim. Then **inferred**, listing any clause built by extrapolation rather than taken
   from the guide, because those are the ones worth checking first.
